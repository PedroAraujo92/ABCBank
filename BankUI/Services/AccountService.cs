...
public class AccountService : IAccountService
{
    ...
    public async Task<ResponseWrapper<int>> AddAccountAsync(CreateAccount createAccount)
    {
    ...
    public async Task<ResponseWrapper<List<AccountResponse>>> GetAccountsAsync()
    {
    ...
    {
        var response = await _httpClient.GetAsync(AccountsEndpoints.GetTransactionsByAccountById(accountId));
        return await response.ToResponse<List<TransactionResponse>>();
    }

    public async Task<ResponseWrapper<int>> TransactionAsync(TransactionRequest transaction)
    {
        var response = await _httpClient.PostAsJsonAsync(AccountsEndpoints.Transaction, transaction);
        return await response.ToResponse<int>();
    }
}
